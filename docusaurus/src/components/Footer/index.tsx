...
export default function Footer({
	...
	const footerLinks =
		(siteConfig.themeConfig.footer as SimpleFooter).links ?? [];
	return (
		<footer className={clsx(styles.footerWrapper, className, 'relative')}>
			{hideCTA ? null : (
				<Link
					href='https://cloud.tembo.io'
					className={clsx(styles.cta, 'bg-transparent border-none')}
				/>
			)}
			<div
			...
			>
				...
				<Link
					className={clsx(styles.joinCTA, 'relative mx-auto mt-20')}
					target='_blank'
