...
The helper functions are in place, we can execute them sequentially.
...
Now we can pass that single image into our `get_image_embeddings` function and then search for similar images.
...
```text
('B086QB7WZ1.jpg', 0.26320752344041964)
('B00FRSYS12.jpg', 0.2626421138474824)
```
...
We've shown how to build a multi-modal search engine on Postgres.
 As a reminder, of the code in this blog is available in [Tembo Github repository](https://github.com/tembo-io/tembo/blob/main/examples/image-search/demo.ipynb).
