---
slug: image-search
title: 'Build an image search engine on Postgres'
authors: [adam]
description: |
...
One of the more interesting e-commerce features I've seen recently is the ability to search for products that look like an image I have on my phone.
 ...
 Getting started with a feature like this can be a fairly straight forward project the right tools,
...
In this blog we'll build a basic image search engine using Postgres.
 We'll use a pre-trained model to generate embeddings for images and text, then store those embeddings in Postgres.
 We'll use the `pgvector` extension to conduct similarity searches on these embeddings.
 Finally we'll use the `pgvector` extension to conduct similarity searches on these embeddings using both images and raw-text as queries.
...
All the code in this blog is available in a notebook in the [Tembo Github repository](https://github.com/tembo-io/tembo/blob/main/examples/image-search/demo.ipynb).
 We'll run through the code in that notebook here.
 It works best to clone the repository, and the easiest way to follow along is to run Postgres and the accompanying Jupyter notebook in Docker.
...
Navigate to http://localhost:8888 and open the `demo.ipynb` notebook.


## Loading Postgres with Image Embeddings

First we need to acquire the raw images.
 We are using an [Amazon Products](https://www.kaggle.com/datasets/spypsc07/amazon-products?resource=download) dataset from Kaggle.
...
Now that we have acquired our sample images, we need to generate embeddings for them.
...
We'll use the CLIP model to generate embeddings for each image, and save them into a Postgres table.
 We'll create a few helper functions for loading our images, generating embeddings, and getting them inserted into Postgres.
...
Now that the helper functions are in place, we can execute them sequentially.
...
for product_id, score in results[:2]:
...
Searching for images that are similar to one another can be very helpful when searching for products.
 However, sometimes people will want to search for images given a raw text string.
...
We've shown how to build a multi-modal search engine on Postgres.
