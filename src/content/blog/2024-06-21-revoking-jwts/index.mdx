...
JSON Web Tokens (JWTs) have become a widely adopted standard for application authentication and authorization. In this blog, we'll explore how to use JWT tokens in your backend services, how third parties like Clerk issue JWTs for your users, and various techniques for handling authentication and authorization.
...
This can be done by maintaining a blacklist of invalidated JWT IDs (jti) and checking each request against this list to ensure the token has not been previously invalidated. The backend must remember which tokens to deny, even though they are still technically valid.
