...
In the above image, we can see the status of the Tembo Cloud instance _over time_. This is an example of a feature that needs [time series data](https://en.wikipedia.org/wiki/Time_series).
...
In the **metric_labels** table, each row is unique by `(name, labels)`, so that we know which metric name and set of labels each data point has. For example, metrics may be labeled by instance ID, organization ID, and so on. Each set of labels for a given metric name represents an independent time series history.
...
This data model helps us efficiently store time series data for this simple use case. As our dataset gets larger and queries complex, we'll need more features like compression, long-term retention, and analytical functions.

At Tembo, we've built a [Time Series Stack](/blog/pg-timeseries) which solves exactly this problem and helps users work with time series data more efficiently with PostgreSQL. You can store your time series data in Postgres by trying the Time Series Stack on Tembo Cloud.

We will soon be migrating this system to the time series stack to use all the time series features Postgres and it's extensions can provide. You can look forward to our blog covering that journey.

## Prometheus Foreign Data Wrapper

We also developed a Postgres extension that helps query Prometheus from inside of Postgres. Please take a look at [prometheus_fdw](https://pgt.dev/extensions/prometheus_fdw) on [GitHub](/blog/monitoring-with-prometheus-fdw), or read about it in the [blog](/blog/monitoring-with-prometheus-fdw).
