...
As data progresses through the various stages of its lifecycle, so too do its access patterns change. Upon injestion and frequent querying, data is said to be at a "hot" stage. While the following is of course circumstantial, it's safe to assume that as this data ages, its frequency of access decreases. Both literally and metaphorically, the data cools and eventually finds itself in "cold" storage.

It's not simply where the data is stored, but a combination of its location how it is formatted. 
