...
While `pg_tier` is a relatively new project, it heavily depends on a long-standing and popular project: `parquet_s3_fdw`. `pg_tier` uses `parquet_s3_fdw` to create a foreign data wrapper around data in S3. This is the key piece of technology that allows you to query the data in S3 as if it were in Postgres.

In order to make the experience optimal for cloud users, every Tembo instance comes with pre-configured S3 as scratch space. As a result of Tembo's contribution to `parquet_s3_fdw`, users are don't need any AWS credential configuration to access their S3 data. We are currently running a public fork of `parquet_s3_fdw` in Tembo Cloud so that you can use this feature today. Altogether, this grants users the Tembo touch-free experience.
