...
At Tembo, we've heard this pain point come up frequently in both internal and community discussions. This was such a common topic, in fact, that we decided to build and open source a community solution, `pg_tier`; a PostgreSQL extension designed to streamline the developer interaction with AWS S3 and other object stores.
...
While we find this exciting in its own right, it's important to appreciate the context within which this extension operates. That is to say, the fundamentals of data lifecycle management and some example use cases.

Access pattern of the data sets varies over their lifecycle. In general, trends of access frequency decrease over the period of time. Pg_tier is envisioned to provide a substantial solution towards data management when it enters the less accessed usage stage (aged data stage) to rarely accessed stage (archival stage).
