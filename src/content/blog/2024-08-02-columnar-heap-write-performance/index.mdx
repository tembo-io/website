...
We should begin with the test table. Since the demonstration at the talk used the Chicago Divvy data, we elected to mimic that data for these tests as well. It presumably models a real-world use case rather than some contrived scenario, and it's on-topic for the talk itself.
...
This would run 20 tests starting at 250,000 rows and ending at 5-million rows (250,000 * 20). This lets us see if row count affects timings and to what extent. The results are categorized by row count, storage type, and write action, so we can slice and dice it several different ways as well.
...
The data we collected used the same parameters as in the above example: 20 samples at 250k rows per step, for a total maximum of 5M rows on the last iteration. We split the results into inserts, updates, and deletes for both Heap and Columnar storage types and graphed the results.
...
Columnar performance is still better, but not by much. This shouldn't really come as a huge surprise as columnar storage isn't really meant to be updated since it's designed for ingesting time-series data. Modifying historical data in columnar storage likely isn't as trivial as the usual delete + insert operation we may see in heap storage.
