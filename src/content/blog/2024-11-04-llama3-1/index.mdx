---
slug: llama-3.1
title: 'Llama 3.1 and 3.2'
authors: [chuck]
description: |
    We've upgraded Tembo Cloud to support Llama 3.1-8B-Instruct
tags: [tembo, ai]
...
In summary - the Llama 3.2 series introduced new capabilities, but not necessarily and improvement to text-text performance. It introduced vision capabilities on top of the text capabilities of the 3.1 series.
...
While Tembo Cloud hosts the Llama 3.1 8B-Instruct model for all users, self-hosted users can run any of these models.
 ...
 So roughly, to host a 70B parameter model you will need 168GB of GPU memory, while an 8B parameter model will need 19GB of GPU memory.
...
It is a large jump in memory requirements to go from the 8B to the 70B models, but is it worth it?
 ...
 The leader board evaluates models across a number tasks involving mathematical reasoning, language understanding, world knowledge, and ability to follow instructions.
...
We've extracted all the `meta-llama` from the [Open LLM Leader Board](https://huggingface.co/spaces/open-llm-leaderboard/open_llm_leaderboard) and visualized below.
...
For development purposes we recommend using our alias, `tembo-small-instruct` which will always route to the latest small instruct model.
