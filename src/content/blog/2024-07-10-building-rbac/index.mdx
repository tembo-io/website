...
In our initial design, we are requiring Policies to be unique by (organization_id, role, action). This differs from the design of AWS IAM, as in IAM you can create multiple policies for the same Role and Action. We believe it is easier to understand Policies when there is only one Policy associated with a particular Role for each Action. This data model better represents a UI that shows a toggle on/off state for each Action when toggling permissions on or off per Role.
