...
title: 'pg_search from ParadeDB: Benchmarks'
authors: [jason]
description: |
    Speed up your full text search with pg_search from ParadeDB!
tags: [tembo, search, paradedb, benchmarking]
date: 2024-11-07T09:00
planetPostgres: false
---

We've previously written about the analytics capabilities of Tembo's ParadeDB integration, but did you know about ParadeDB's lightning-fast search capabilities as well? ParadeDB provides `pg_search`, a complete full-text search solution for PostgreSQL built atop [BM25](https://en.wikipedia.org/wiki/Okapi_BM25) and [Tantivy](https://github.com/quickwit-oss/tantivy).
...
For this benchmark, I used ParadeDB's own [`cargo-paradedb`](https://github.com/paradedb/paradedb/tree/dev/cargo-paradedb) tool to generate workloads and queries. It wraps a Go tool which generates the Elastic OpenSearch Benchmark corpus and which can create GIN and BM25 indexes for benchmarking.
...
![Full Text Search Results, 10MM Rows](./GIN-10MM-PDF.svg)

#### ParadeDB's `pg_search`

The ParadeDB implementation answered a comparable query within **6.28ms**, _nearly 265 times faster_. Note that the probability distribution function reflects that these results are much more consistent than the full text ones, as well…

![pg_search Results, 10MM Rows](./search-10MM-PDF.svg)
...
![Full Text Search Results, 100MM Rows](./GIN-100MM-PDF.svg)
...
![pg_search Results, 10MM Rows](./search-100MM-PDF.svg)
