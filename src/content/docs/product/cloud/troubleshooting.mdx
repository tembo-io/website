...
Tembo uses self-signed certificates for databases. The [standard practice for Postgres servers](https://www.postgresql.org/docs/current/ssl-tcp.html#SSL-CLIENT-CERTIFICATES) is to self-sign certificates, managing authentication by specifying trusted root certificates.
