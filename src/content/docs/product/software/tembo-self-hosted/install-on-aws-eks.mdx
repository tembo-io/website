...
## Step 2: Create an EKS Kubernetes cluster
...
## Step 3: Prepare for Tembo installation
...
## Step 4: Install the Tembo Self Hosted Helm chart
...
To properly route traffic to your Traefik load balancer, you need to configure a DNS A record through your DNS provider. Follow the steps below to create a wildcard A record, which is essential because each PostgreSQL instance has its own subdomain for its connection string.

1. **Create a Wildcard A Record**: Set up a wildcard A record (e.g., `*.tembo.mydomain.com`). This will cover all subdomains for your PostgreSQL instances.

2. **Use the External IP**: Point the wildcard A record to the external IP of your Traefik load balancer. This external IP can be found using the command below.
...
4. **Verify DNS Settings**: After creating the A record, verify that the DNS settings are correctly propagating and resolving.
...
## Step 6: Access the Tembo Self Hosted
