...
<Callout variant='info'>
	The `->` operator returns data as the jsonb type. The operator `->>` returns
	the data as text.
</Callout>
...
<Callout variant='info'>
	PL/pgSQL stands for "Procedural Language / PostgreSQL", and it's the
	PostgreSQL database's default procedural language.
</Callout>
...
<Callout variant='info'>
	Prefixing a command by `EXPLAIN ANALYZE` will run the query and show the
	query plan.
</Callout>
...
<Callout variant='warning'>
	Running `CREATE INDEX .. ON users ...` locks the users table. Instead, use
	`CREATE INDEX CONCURRENTLY`, which is mostly non-blocking. However, it's
	also slower to create the index, and if it fails part way through, it will
	leave behind an invalid index.
</Callout>
