...
Tembo simplifies using Postgres with a virtualized runtime experience, enabling one-click deployments of the entire Postgres ecosystem. Developers can control the data model lifecycle, and deploy to multiple zones and clouds. Advanced options will include autoscaling, hybrid transactional and analytical processing (HTAP), and edge caching.
...
Tembo Stacks accelerate your development by enabling you to quickly deploy (and author, if you're adventurous) custom-built "flavors" of Postgres + extensions that are tailor-made for key use cases. No need to spin up new vendors and endure the pain and associated sprawl—Tembo Stacks enable you to replace external services.
...
<Callout title='Try it out!' variant='tip'>
